...
export function ProductMediaManager({
  ...
  async function uploadFiles(files: FileList | null) {
    ...
    try {
      ...
      for (const [index, file] of selected.entries()) {
        const path = `products/${productId}/${crypto.randomUUID()}.${extensionFor(file)}`;
  ...
  async function persistOrder(nextItems: ProductMediaItem[]) {
  ...
  async function remove(item: ProductMediaItem) {
    if (!window.confirm("Bu fotoğrafı üründen kalıcı olarak silmek istiyor musunuz?")) {
      return;
    }
    ...
    try {
      const remaining = items.filter((candidate) => candidate.mediaId !== item.mediaId);

      if (item.isCover && remaining.length > 0) {
        const { error: coverError } = await supabase.rpc("set_product_cover", {
          p_product_id: productId,
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-extrabold">Ürün fotoğrafları</h2>
      ...
      {items.length > 0 ? (
          ...
          {items.map((item, index) => (
            ...
            >
              <div
                ...
                style={{ backgroundImage: `url("${publicUrl(item.storagePath)}")` }}
              />
              <div className="p-4">
        ...
        <div className="mt-6 rounded-card border border-dashed border-border bg-surface-muted p-8 text-center">
          <p className="font-extrabold">Henüz ürün fotoğrafı yok.</p>
